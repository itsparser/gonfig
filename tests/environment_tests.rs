use gonfig::{Environment, ConfigSource};
...
fn test_environment_type_parsing() {
    ...
    let result = env.collect().unwrap();
    assert_eq!(result.get("test_float").unwrap().as_f64(), Some(3.14));
...
fn test_environment_overrides() {
    ...
    let env = Environment::new()
        .override_with("OVERRIDE_TEST", "overridden");
    
    let result = env.collect().unwrap();
    assert_eq!(result.get("override_test").unwrap().as_str(), Some("overridden"));
