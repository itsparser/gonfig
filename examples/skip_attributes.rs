...
fn print_app_config(config: &AppConfig) {
    ...
    println!("   Runtime Client: {:?} (skipped, set to None)", config.runtime_client);
    println!("   Internal State: {:?} (skipped, empty)", config.internal_state);
...
fn print_app_config_with_skipped(config: &AppConfig) {
    ...
    println!("   Thread Pool: [MANUALLY INITIALIZED - {} threads]", 
...
fn print_db_config(config: &DatabaseConfig) {
    ...
    println!("   Connection Pool: {:?} (skipped, None)", config.connection_pool);
}
